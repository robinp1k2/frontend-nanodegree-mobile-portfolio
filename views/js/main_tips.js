...
//TODO - delete function determineDx...
/*
  ...
  function changePizzaSizes(size) {
	...
	switch(size) {
		...
		case "3":
			...
			break;
		default:
			console.log("error in function changePizzaSizes.");		
	}
	var pizzaBoxes = document.querySelectorAll(".randomPizzaContainer");
    for (var i = 0; i < pizzaBoxes.length; i++) {
...
function updatePositions() {
  ...
  /* Is there a faster way to access the DOM than querySelectorAll? 4:01/15:52 */
  var items = document.querySelectorAll('.mover'); //getting all the pizzas that are animating are in the class 'mover'
  // TODO more efficient way to access DOM -> document.getElementsByClass()
  for (var i = 0; i < items.length; i++) { // On each pizza
	  /*TODO What are the exact numbers that phase and document.body.scrollTop give me per iteration?
	  
	  Futhermore we see that the phase value depends on the modulo operator '%'.  Modulo gives us athe remainder when we divide i by 5.
	  Therefore we are calculating the same set of 5 numbers for all of our pizzas no matter how big our list of pizzas are!
	  */
    var phase = Math.sin((document.body.scrollTop / 1250) + (i % 5)); //do a calculation using i but could take "scrollTop" call outside the loop - store it in a variable ahead of time
	// Let's log our all these numbers and see!  It will be just 5 numbers repeated
	//Just store 5 numbers and use those numbers to change the position of our element (without doing any calculations)
	// console.log(phase, document.body_scrollTop / 1250);
	...
	Perhaps CSS3 hardware acceleration can reduce the need to trigger a re-layout?  Can we offload the CPU and 
	The CSS 'transform' property can help us here.
	*/
	//CSS3 has hardware accesleratio and certain transformations that reduce the need to trigger a re-layout
	//example is transform:  translateX();  Instead of using .style.left you could actually use translateX 
	//so this gets rid of the need to trigger a re-layout.
    items[i].style.left = items[i].basicLeft + 100 * phase + 'px';  //change the left position on each pizza
  }
  
  /* Advanced Hack Here:  Can we also reduce the need for the browser to paint the entire screen?  Can we tell 
  actually moving?  Whenever a pixel in a layer changes, the browser repaints the entire layer.  Thereforeanimating pizza in its own layer?  Therefore whenever we animate the pizzas, only a small part of the screen
  
  We should look up these CSS Hacks to see if they can force our elements into its layer:
  transform: translateZ(0);
  transform: translate3d(0,0,0);
  backface-visibility: hidden; -> use this to render each pizza into its own layer.  Do it by putting it in the css class.
  
  Be careful of these hacks:
  
  This hack and wreak havoc on mobile devics due to the low VRAM for some mobile devices:
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  /* I could only crete a handful of pizzas that show up on the screen at any given scroll.  That amount doesn't look */
  for (var i = 0; i < 200; i++) {
